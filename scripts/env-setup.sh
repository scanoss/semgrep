...
# Config goes into: /usr/local/etc/scanoss/semgrep
# Logs go into: /var/log/scanoss/semgrep
...
if [ "$1" = "-h" ] || [ "$1" = "-help" ] ; then
  ...
  echo "   Setup and copy the relevant files into place on a server to run the SCANOSS SEMGREP API"
...
CONF_DIR=/usr/local/etc/scanoss/semgrep
LOGS_DIR=/var/log/scanoss/semgrep
CONF_DOWNLOAD=https://raw.githubusercontent.com/scanoss/semgrep/main/config/app-config-prod.json
...
if [ "$FORCE_INSTALL" -eq 1 ]; then
  ...
  read -p "Install Semgrep API $ENVIRONMENT (y/n) [n]? " -n 1 -r
  echo
  if [[ $REPLY =~ ^[Yy]$ ]]; then
...
echo "Setting up Semgrep API system folders..."
...
SC_SERVICE_FILE="scanoss-semgrep-api.service"
SC_SERVICE_NAME="scanoss-semgrep-api"
if [ -n "$ENVIRONMENT" ] ; then
  SC_SERVICE_FILE="scanoss-semgrep-api-${ENVIRONMENT}.service"
  SC_SERVICE_NAME="scanoss-semgrep-api-${ENVIRONMENT}"
fi
...
if ! cp $SCRIPT_DIR/scanoss-semgrep-api.sh /usr/local/bin ; then
  echo "Semgrep api startup script copy failed"
  exit 1
fi
if ! chmod +x /usr/local/bin/scanoss-semgrep-api.sh ; then
  echo "Semgrep api startup script permissions failed"
  exit 1
fi
...
BINARY=scanoss-semgrep-api
if [ -f "$SCRIPT_DIR/$BINARY" ] ; then
  echo "Copying app binary to /usr/local/bin ..."
  if ! cp $SCRIPT_DIR/$BINARY /usr/local/bin ; then
    echo "copy $BINARY failed"
    echo "Please make sure the service is stopped: systemctl stop scanoss-semgrep-api"
    exit 1
  fi
  if ! chmod +x /usr/local/bin/$BINARY ; then
    echo "execution permission on $BINARY failed"
    echo "Please make sure the use can set the binary as executable: chmod +x /usr/local/bin/scanoss-semgrep-api"
  fi
else
  echo "Please copy the Semgrep API binary file into: /usr/local/bin/$BINARY"
fi
...
echo "Count the number of running scans using: pgrep -P \$(pgrep -d, scanoss-semgrep-api) | wc -l"
echo
